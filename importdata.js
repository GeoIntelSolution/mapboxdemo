...
Object.keys(source.sources).map((k,i)=>{
    // console.log(k);
    const handle =source.sources[k];
    const url=handle["tiles"][0];
    let pats =url.split("&");
    // console.log(pats);
    let target=pats[3];
    const lenth =target.length;
    target=target.slice(6,lenth);
    console.log(target);
    // console.log(url);
    // console.log(url.findIndex("LAYER"));
    console.log(i)
})